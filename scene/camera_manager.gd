...
@onready var back_ground: Sprite2D = get_node("Sprite2D")
...
func _process(_delta: float) -> void:
	...
	back_ground.scale = Vector2.ONE / zoom
...
func get_end_point_size(end_point) -> float:
	var ball_size: float = (
		end_point.sprite.get_rect().size.y * end_point.scale.y * end_point.sprite.scale.y
	)
	return ball_size
